 ...
 * No Android or camera dependencies â€” accepts raw grayscale byte arrays directly.
 *
 * Usage:
 *   LineFollowerPipeline pipeline = new LineFollowerPipeline();
 *   int[] result = pipeline.processFrame(grayscaleBytes, width, height);
 *   // result[0] = Steering  [-100 â€¦ +100]  (negative = line left, positive = line right)
 *   // result[1] = Tracking  [  0 â€¦ 100  ]  (100 = straight path ahead, 0 = hard turn / line lost)
 *
 * Algorithm:
 *   1. Build a grayscale histogram of the Horizontal ROI and run Otsu's method
 *      (from scratch â€” no OpenCV) to pick the threshold that best separates the
 *      track from the floor under the current lighting.
 *   2. Threshold: pixels above (Otsu Â± bias) are "track" (bright = white line).
 ...
 *      and the band is followed to the frame edge â†’ steering [-100 â€¦ +100]. When
 *      the band finally leaves the gate, steering pins to that edge (Â±100) and a
 *      new line is re-acquired (full-width) only after a short grace period.
 *   4. V-ROI look-ahead â†’ tracking value [0 â€¦ 100]
...
public final class LineFollowerPipeline {
    ...
    public static final class Config {
        ...
        // miss its far end â€” the tracker then loses the line early and the track
        ...
        // the âŠ¥) is kept entirely ABOVE this band, so robot parts never enter the
        ...
        // value only climbs for harder turns. 30Â° is a responsive default.
        public float curveFullScaleDeg = 30f;

        // â”€â”€ Perspective / mounting geometry â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // The phone is on a fixed mount tilted so its optical axis is depressed
        // this many degrees below horizontal (45Â° = halfway between straight
        ...
        // the image â€” and a flat track of constant real width projects narrower
        ...
        // back cameras are ~55â€“65Â° vertical; 60Â° is a safe default. Tune if known.
        public float verticalFovDeg = 60f;

        // â”€â”€ Track memory / continuity â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // been gone for trackLostGraceFrames â€” i.e. once you have moved fully off
        // it. Turn OFF to fall back to a memoryless full-width search every frame.
        public boolean trackMemory = true;

        // Gate half-width = trackMemoryGateMult Ã— the measured track width, with a
        // floor of trackMemoryMinGatePx. Wider = tolerates faster sideways motion
        // but lets nearer reflections in; tighter = stricter rejection. 3Ã— the line
        ...
        // edge it left (Â±100) and the turn signal is forced to 100 %. After this
        // many consecutive missing frames the lock is dropped and a full-width
        // search re-acquires a new line. ~8 frames â‰ˆ 0.25 s at 30 fps.
        public int trackLostGraceFrames = 8;

        // â”€â”€ Track value (look-ahead position) calibration â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // The "track" output is STEERING applied to the LOOK-AHEAD â€” the purple
        // V-ROI in front of the robot â€” using the same band-lock memory as
        ...
        //â”€â”€ Auto-calibration (camera mounting offset) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // detections, record where the near line (steering) actually sits â€” that
    ...
    /**
     * All geometry and state data the engineer needs to draw a visual overlay.
    ...
    public static final class FrameData {
        ...
        /** True once the straight-ahead calibration reference has been captured. */
    ...
    // â”€â”€ Track memory (line-lock) state â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Auto-calibration state (camera mounting offset) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    // steerRefX / trackRefX are the image columns that count as "straight ahead"
    // for the near (steering) and look-ahead (track) measurements â€” captured once
    ...
    // â”€â”€ Dynamic look-ahead path (for the overlay to follow the road) â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // H-ROI row. A fixed real lateral offset â€” the camera's mount offset â€” projects
    ...
    // Image row (fractional, 0=top â€¦ 1=bottom) of the floor's horizon / vanishing
    // line, derived from the mount tilt. May be negative (horizon above the frame),
    // which is expected for a steep downward tilt like 45Â°.
    private float horizonYFrac;

    // â”€â”€ Frames-per-second measurement (perf tracking) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // Second histogram for the look-ahead (V-ROI) Otsu â€” the far region is dimmer,
    ...
    // Callback registered by the engineer — null if not set.
     ...
     *
     * The callback fires after every {@link #calibrate} call and after every
     * {@link #processFrame} call, so the engineer can drive their overlay from
     * a single listener without polling any getters manually.
     *
     ...
     *
     * @param rawGrayscaleData  packed grayscale pixels, row-major, one byte per pixel (0â€“255).
     *                          Array length must be >= width * height.
     * @param width             frame width  in pixels
     * @param height            frame height in pixels
     * @return int[2] (same array instance reused each call â€” copy if you need to keep values):
     *         [0] = Steering  [-100 â€¦ +100]  negative = line is left, positive = line is right
     *         [1] = Tracking  [  0 â€¦ 100  ]  100 = straight path ahead, 0 = hard turn / line lost
     */
    public int[] processFrame(byte[] rawGrayscaleData, int width, int height) {
        ...
        //   internal 0   (straight) â†’ output 100 (no braking needed)
        //   internal 100 (hard turn / lost) â†’ output 0 (brake / slow down)
        result[1] = Math.max(cfg.trackingMinValue, Math.round(100f - output.intersection));
        fireCallback(result[0], result[1], output.trackFound);
        return result;
    }
    ...
    public boolean calibrate(byte[] rawGrayscaleData, int width, int height) {
        ...
        // Fire the callback so the engineer can show calibration progress in their overlay
        final boolean lineFoundThisFrame = bestSum >= minWeight && bestStart >= 0;
        fireCallback(0, 100, lineFoundThisFrame);
    ...
    public void init(int width, int height, Config config) {
        ...
        // from centre toward the top:  horizonYFrac = 0.5 âˆ’ tilt/fov.
        ...
        // (weight = vRoiBot âˆ’ row), so the "effective" row is the weight-centroid of
        ...
        // road â‡’ track ~0, curve â‡’ genuine turn.
        {
            final double a = vRoiTop, b = vRoiBot;
            final double wSum  = (b - a) * (b - a) * 0.5;                          // âˆ«(bâˆ’r)dr
            final double rwSum = b * (b * b - a * a) / 2.0 - (b * b * b - a * a * a) / 3.0; // âˆ«r(bâˆ’r)dr
    ...
    // Geometry getters â€” useful for any host UI that needs to draw sensor overlays.
    ...
    // â”€â”€ Dynamic look-ahead path (nearest â†’ farthest), fractional image coords â”€â”€
    ...
    /** Fractional image row (0=topâ€¦1=bottom) of the floor horizon from the mount tilt. */
    ...
    // =========================================================================
    //  Hot path â€” zero heap allocations per frame
    // =========================================================================
    ...
    private void runPipeline(byte[] pixels, int width, int height) {
        ...
        // â”€â”€ Stage 1: Otsu adaptive threshold from the H-ROI histogram â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        // â”€â”€ Stage 2: gated line-lock â†’ steering â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        //
        //   Frame layout (row 0 = top = farthest floor, bottom = nearest robot):
        //
        //   row 0   â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
        //           â”‚   [V-ROI] narrow column above    â”‚
        //   hRoiTop â• â•â•â•â•â•â•â•â•â•â•[ H-ROI ]â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•£  â† scan strip
        //   hRoiBot â•šâ•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
        //
        ...
        // white band â€” the track. With track memory on we only scan a gate around
        ...
        boolean trackFound;
        int     centroidX;
        float   forcedIntersection = -1f;   // â‰¥ 0 overrides the look-ahead stage
        ...
        if (bestSum >= minWeight && bestStart >= 0) {
            ...
            // The line we were following just left the gate â†’ you have moved fully
            ...
            // a fully-lost line still reads steering Â±100 after the offset below.
            ...
            if (lostFrames > cfg.trackLostGraceFrames) {
                trackAcquired = false;     // release lock â†’ full-width re-acquire next frame
            }
        } else {
            // No lock and nothing found â€” blank view; hold neutral.
            trackFound = false;
        ...
        // â”€â”€ Stage 3 threshold: adaptive to the (dimmer, farther) look-ahead â”€â”€â”€
        // The far look-ahead line is dimmer than the near line, so the H-ROI Otsu
        // can miss it â€” the tracker then loses the line early and the track value
        ...
        // â”€â”€ Stage 3: look-ahead straightness / curvature â†’ intersection â”€â”€â”€â”€â”€â”€â”€
        //
        ...
        // centroids we derive ONE 0â€¦100 signal where:
        //
        //     0   = the track goes perfectly straight ahead (straight up the
        //           frame, aligned with the robot â€” mirrors steering == 0),
        //   100   = a hard turn / the track bends sharply away.
        //
        // It is the larger of two terms, so it responds to BOTH rotation and curves:
        //   â€¢ angle = |tilt of the look-ahead line from vertical|  â†’ grows as the
        //             robot rotates / the path leans left or right.
        //   â€¢ bend  = how far the centroids depart from a straight line
        //             â†’ grows with genuine curvature beyond a straight lean.
        //
        ...
        //   â€¢ minRowW   â€” a row's white run must be at least this many pixels to
        //                 count, so 1-px specular noise never seeds the tracker.
        //   â€¢ vMaxDrift â€” the run's centre may sit at most this far from where the
        ...
        //     position on the row below â€” never the average of all white in the
        //     window â€” and reject any run that is too small or too far (a different
        ...
        // Dynamic look-ahead path (nearest â†’ farthest) for the overlay to follow.
        ...
        for (int row = vRoiBot - 1; row >= vRoiTop && trackFound; row--) {
            ...
            // are following; otherwise this row has no usable track â†’ count a gap.
            if (bestW >= minRowW && bestDist <= vMaxDrift) {
                rowCx[row]  = (float) bestC;
                trackCenter = (int) (bestC + 0.5);        // follow the line upward
                gap = 0;
                final double laWeight = vRoiBot - row;    // farther ahead â†’ weighted more
                ...
                if (++gap > maxGap) break;                 // line ended â†’ stop look-ahead
            }
        }
        laPathCount = laCount;   // publish the path for any external overlay

        // 3b. Line tilt from the least-squares slope  b = d(col)/d(row) = tan(Î¸).
        //     Kept only to lean the V-ROI overlay box with the line â€” it no longer
        ...
        // â”€â”€ Steering: signed offset from the calibrated straight-ahead column â”€â”€
        //    Same per-pixel sensitivity as before (Ã·halfW), just zeroed at the
        ...
        // 3d. Intersection â€” internal scale (0 = straight, 100 = hard turn / lost).
        ...
        if (!trackFound || !lookAheadValid) {
            intersection = 100f;                          // no road ahead â†’ off
        } else {
        ...
        // â”€â”€ Write internal output â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
     ...
     * starts â€” so {@link #getFramesPerSecond()} always reflects the last whole
     * second, not a cumulative total.
     */
    private void fireCallback(int steering, int tracking, boolean trackFound) {
        if (frameCallback == null) return;
        final FrameData d = frameData;
        d.hRoiTopFrac  = (imgH > 0) ? hRoiTop / (float) imgH : 0f;
        d.hRoiBotFrac  = (imgH > 0) ? hRoiBot / (float) imgH : 0f;
        d.vRoiTopFrac  = (imgH > 0) ? vRoiTop / (float) imgH : 0f;
        d.steerRefXFrac = (imgW > 0) ? steerRefX / imgW : 0.5f;
        d.trackRefXFrac = (imgW > 0) ? trackRefX / imgW : 0.5f;
        // Box half-widths with perspective taper — uses actual detected track width
        final float boxW     = Math.max(12f, lastTrackWidthPx * 1.3f);
        d.halfBotFrac   = (imgW > 0) ? (boxW * 0.5f) / imgW : 0.1f;
        ...
        // Centroid derived from steering value and calibrated reference
        d.centroidXFrac = (trackFound && imgW > 0)
                ? (steerRefX + (steering / 100f) * (imgW * 0.5f)) / imgW
                : -1f;
        // Dynamic road path — copy so the engineer's buffer is safe to read
        final int n = Math.min(laPathCount, 48);
    ...
    // =========================================================================
    //  Otsu's method  (custom, from scratch â€” no OpenCV)
    // =========================================================================
     ...
     * splits the pixels into two classes â€” floor and track â€” so that the
     ...
     *
     *     Ïƒ_bÂ²(t) = w0(t)Â·w1(t)Â·(Âµ0(t) âˆ’ Âµ1(t))Â²
     *
     * which is the cheap single-pass form computed here (the total variance is
     * constant, so the t that maximises Ïƒ_bÂ² is exactly the t that minimises the
     * weighted within-class variance). w0/w1 are the class pixel counts and
     * Âµ0/Âµ1 their mean grey levels. We track running sums so the whole sweep is
    ...
    private static int otsuThreshold(int[] hist, long total) {
        ...
        // Weighted sum of all grey levels: Î£ gÂ·hist[g].
        double sumAll = 0.0;
        for (int g = 0; g < 256; g++) {
            sumAll += (double) g * hist[g];
        }

        double sumBack = 0.0;   // Î£ gÂ·hist[g] for the background class [0..t]
        long   wBack   = 0;     // pixel count of the background class
        double bestVar = -1.0;
        int    bestT   = 128;

        // Sweep every candidate threshold, moving level t from backgroundâ†’foreground.
        for (int t = 0; t < 256; t++) {
            wBack += hist[t];
            if (wBack == 0) continue;          // background still empty
            long wFore = total - wBack;        // foreground class (t+1 .. 255)
            if (wFore == 0) break;             // foreground emptied â€” done
