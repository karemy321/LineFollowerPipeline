...
public final class LineFollowerPipeline {
    ...
    private void runPipeline(byte[] pixels, int width, int height) {
        ...
        // ── One-time auto-calibration of the camera mounting offset ───────────
        // Assuming the robot starts correctly placed, collect the NEAR-line column
        // over the first calibFrames solid detections and take its MEDIAN as the
        // steering zero — robust to the odd bad frame, so a correctly-placed robot
        // reads steering 0. The look-ahead (track) zero is DERIVED, not measured:
        // the same camera mount offset that shifts the near line shifts the far line
        // too, but scaled down by perspective (laPerspectiveRatio). So trackRefX is
        // where a STRAIGHT road ahead would project. On a circle the real look-ahead
        // line sits off that reference, so track reads a genuine turn (never forced
        // to 0), while on a straight road it still reads ~0. Only the near line has
        // to be visible to calibrate — the (possibly curved) look-ahead is not used.
        if (!calibrated && trackFound) {
